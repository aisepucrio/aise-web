...
/// Componente reutilizável que renderiza um conjunto de ícones-link (ActionIcon do Mantine) para abrir URLs externas.

// Configurações dos tipos de links suportados
const LINK_CONFIGS = {
  webapp: {
    icon: IconWorld,
  ...
  api: {
    icon: IconCode,
    hoverShadow: "0 4px 12px rgba(139, 92, 246, 0.3)",
    color: "violet" as const,
    aria: "API",
  },
  docs: {
    icon: IconBook,
    hoverShadow: "0 4px 12px rgba(20, 184, 166, 0.3)",
    color: "teal" as const,
    aria: "Documentation",
  },
  linkedin: {
  ...
  lattes: {
    // Inline Lattes SVG so it can inherit color via `currentColor`
  ...
  orcid: {
    // Render 'ID' text so it can inherit color (currentColor) and easily styled
    icon: (props: { size?: number }) => (
      <span
        aria-hidden
        style={{
...
export type LinkType = keyof typeof LINK_CONFIGS;
...
export const LinkGroup: React.FC<LinkGroupProps> = ({
  ...
  // Filtra e mapeia links válidos
  const linkItems = Object.entries(links)
    .filter(([key, url]) => key in LINK_CONFIGS && url)
    .map(([key, url]) => {
      const config = LINK_CONFIGS[key as LinkType];
      const IconComponent = config.icon;

      return {
        href: url,
        aria: config.aria,
        icon: <IconComponent size={isMobile ? 22 : 26} />,
  ...
  return (
    <>
      {/* Group do Mantine usado para alinhar os ActionIcons.*/}
      <Group gap="sm" justify={justifyMap[align]} className={className}>
